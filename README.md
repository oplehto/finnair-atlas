...
`public/og-preview.png` is the social card image, 1200×630, rendered by hand from a built sheet: a crop of Copenhagen's bundles climbing into Helsinki's west edge, where the flight numbers, day marks, equipment codes and hub brackets are all legible. The whole sheet at fit reads as texture at card size; this shows what the sheet actually is. It is referenced by absolute URL and keeps a stable name, because the platforms that scrape it cache by URL and a changing name would orphan every post already shared. It is the one artifact in the repository that does not regenerate itself, so refresh it when the sheet's design changes.
...
`.github/workflows/pages.yml` publishes the `site/` build to <https://oplehto.github.io/finnair-atlas/> on every push to `main`, running `npm ci && npm test && npm run build` first, so a failing test blocks the publish. `.github/workflows/ci.yml` runs the same tests on pull requests. Nothing else is needed: no deploy token, no third-party host.

...
In addition to the direct network, the atlas includes 58 major global codeshare connections (116 flights) via key oneworld partners Qatar Airways (via Doha `DOH`), Qantas (via Singapore `SIN`), American Airlines (via Miami `MIA`, Dallas `DFW`, Los Angeles `LAX`), Alaska Airlines (via Seattle `SEA`), Japan Airlines (via Tokyo Haneda `HND`), and Cathay Pacific (via Hong Kong `HKG`):

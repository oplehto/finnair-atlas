...
test('serves an explicitly illustrative validated schedule',async()=>{const r=await request('/api/schedule');assert.equal(r.status,200);const data=validateSchedule(JSON.parse(r.body));assert.equal(data.demo,true);assert.equal(data.flights.length,demo.flights.length);assert.equal(data.title,'Finnair, by air');});
